...
class MapDatasetNuisanceE(MapDataset):
    ...
    def exposure_modified(self):
        if self.N_parameters is not None:
            #self.exposure.data = self.rel_3d(data = self.exposure.data , 
            #                                t = self.N_parameters['effareatilt'].value,
            #                                s = self.N_parameters['effareashift'].value) 
    ...
    def npred(self):
        ...
        if self.background:
            npred_total.data += self.rel_3d(data = self.npred_background().data , 
                                            t = self.N_parameters['effareatilt'].value,
                                            s = self.N_parameters['effareashift'].value) 
            #npred_total.data += self.npred_background().data
    ...
    def stat_sum(self):
        ...
        if self.penalty_sigma and self.penalty_sigma[0]>0.:
            penalty = self.N_parameters[0].value**2 / self.penalty_sigma[0] **2 +  self.N_parameters[1].value**2 / self.penalty_sigma[1] **2
        else:
            penalty = 0
